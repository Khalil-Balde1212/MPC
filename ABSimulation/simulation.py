...
def plot_open_vs_closed(t, omega_open, omega_closed, omega_ref_rpm, u_open=None, u_closed=None):
    ...
    axs[0].plot(t, omega_open, "--", label="Open-loop (u = ωref/K, clamped)")
    axs[0].plot(t, omega_closed, "-", label="Closed-loop (PI + saturation)")
    axs[0].axhline(omega_ref_rpm, linestyle=":",
                   linewidth=1, color="k", label="Reference")
    axs[0].set_xlabel("Time [s]")
    axs[0].set_ylabel("Speed ω [rpm]")
    axs[0].set_title("RPM Response: Open-loop vs PI Closed-loop (0–2.5 V)")
    axs[0].grid(True)
    axs[0].legend()
    # plt.tight_layout()
    # plt.show()
    ...
    if (u_open is not None) and (u_closed is not None):
        ...
        axs[1].plot(t, u_open, "--", label="Open-loop u [V]")
        axs[1].plot(t, u_closed, "-", label="Closed-loop u [V]")
        axs[1].axhline(2.5, linestyle=":", linewidth=1,
                       color="k", label="u_max = 2.5 V")
        axs[1].axhline(0.0, linestyle=":", linewidth=1,
                       color="k", label="u_min = 0 V")
        axs[1].set_xlabel("Time [s]")
        axs[1].set_ylabel("Voltage u [V]")
        axs[1].set_title("Control Voltage (with saturation)")
        axs[1].grid(True)
        axs[1].legend()
        # plt.tight_layout()
        # plt.show()
...
if __name__ == "__main__":
    ...
    t, omega_ol, u_ol = simulate_open_loop_rpm(
        omega_ref, T, dt, K, tau, 0.0, 2.5)
    t, omega_cl, u_cl, e_cl = simulate_closed_loop_incremental_pi_rpm(
        omega_ref, Kp, Ki, T, dt, K, tau, 0.0, 2.5)
